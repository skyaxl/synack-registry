...
var UserWhere = struct {
	...
	UserName: whereHelpernull_String{field: "\"users\".\"user_name\""},
...
type userR struct {
	UserNameRegistry *Registry `boil:"UserNameRegistry" json:"UserNameRegistry" toml:"UserNameRegistry" yaml:"UserNameRegistry"`
}
...
var (
	userAllColumns            = []string{"user_name", "name", "password"}
	userColumnsWithoutDefault = []string{"user_name", "name", "password"}
	userColumnsWithDefault    = []string{}
	userPrimaryKeyColumns     = []string{"user_name"}
)
...
// LoadUserNameRegistry allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-1 relationship.
func (userL) LoadUserNameRegistry(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`registries`),
		qm.WhereIn(`registries.user_name in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		...
		object.R.UserNameRegistry = foreign
		if foreign.R == nil {
			foreign.R = &registryR{}
		}
		foreign.R.UserNameUser = object
	}

	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.UserName, foreign.UserName) {
				local.R.UserNameRegistry = foreign
				if foreign.R == nil {
					foreign.R = &registryR{}
				}
				foreign.R.UserNameUser = local
				break
			}
		}
	}
...
// SetUserNameRegistry of the user to the related item.
// Sets o.R.UserNameRegistry to related.
// Adds o to related.R.UserNameUser.
func (o *User) SetUserNameRegistry(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Registry) error {
	var err error

	if insert {
		queries.Assign(&related.UserName, o.UserName)
		...
		values := []interface{}{o.UserName, related.ID}
	...
	if o.R == nil {
		o.R = &userR{
			UserNameRegistry: related,
		}
	} else {
		o.R.UserNameRegistry = related
	}
	...
	if related.R == nil {
		...
		related.R.UserNameUser = o
	}
	return nil
}

// RemoveUserNameRegistry relationship.
// Sets o.R.UserNameRegistry to nil.
// Removes o from all passed in related items' relationships struct (Optional).
func (o *User) RemoveUserNameRegistry(ctx context.Context, exec boil.ContextExecutor, related *Registry) error {
	var err error

	queries.SetScanner(&related.UserName, nil)
	if _, err = related.Update(ctx, exec, boil.Whitelist("user_name")); err != nil {
		return errors.Wrap(err, "failed to update local table")
	}

	if o.R != nil {
		o.R.UserNameRegistry = nil
	}
	if related == nil || related.R == nil {
		return nil
	}

	related.R.UserNameUser = nil
	return nil
}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, userName null.String, selectCols ...string) (*User, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"users\" where \"user_name\"=?", sel,
	)
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, userName null.String) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"users\" where \"user_name\"=? limit 1)"
