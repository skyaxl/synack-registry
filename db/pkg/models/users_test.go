...
func testUserOneToOneRegistryUsingUserNameRegistry(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &foreign, registryDBTypes, true, registryColumnsWithDefault...); err != nil {
	...
	queries.Assign(&foreign.UserName, local.UserName)
	if err := foreign.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.UserNameRegistry().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadUserNameRegistry(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UserNameRegistry == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.UserNameRegistry = nil
	if err = local.L.LoadUserNameRegistry(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UserNameRegistry == nil {
		t.Error("struct should have been eager loaded")
...
func testUserOneToOneSetOpRegistryUsingUserNameRegistry(t *testing.T) {
	var err error
	...
	for i, x := range []*Registry{&b, &c} {
		err = a.SetUserNameRegistry(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.UserNameRegistry != x {
			t.Error("relationship struct not set to correct value")
		}
		if x.R.UserNameUser != &a {
		...
		zero := reflect.Zero(reflect.TypeOf(x.UserName))
		reflect.Indirect(reflect.ValueOf(&x.UserName)).Set(zero)
...
func testUserOneToOneRemoveOpRegistryUsingUserNameRegistry(t *testing.T) {
	var err error
	...
	if err = a.SetUserNameRegistry(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveUserNameRegistry(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.UserNameRegistry().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if a.R.UserNameRegistry != nil {
		t.Error("R struct entry should be nil")
	}

	if !queries.IsValuerNil(b.UserName) {
		t.Error("foreign key column should be nil")
	}

	if b.R.UserNameUser != nil {
...
var (
	userDBTypes = map[string]string{`UserName`: `VARCHAR(50)`, `Name`: `VARCHAR(100)`, `Password`: `VARCHAR(500)`}
	_           = bytes.MinRead
)
